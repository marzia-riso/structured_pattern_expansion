...
class StableDiffusionExpansionPipeline(StableDiffusionInpaintPipeline):
    ...
    def __call__(
        ...
        tileable: Optional[bool] = True,
        replication_iter : Optional[int] = 35,
        expansion_factor: Optional[Tuple[int, int]] = (1, 1),
        ...
        **kwargs,
        ...
        self._highres = True
        self.replication_iter = replication_iter
        self._patched = True
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):               
                ...
                if (i == self.replication_iter):
                    height = height * expansion_factor[0]
                    width = width * expansion_factor[1]

                    # Low-frequency to high-frequency
                    latents = latents.repeat((1, 1, expansion_factor[0], expansion_factor[1]))
                    mask = mask.repeat((1, 1, expansion_factor[0], expansion_factor[1]))
                    masked_image_latents = masked_image_latents.repeat((1, 1, expansion_factor[0], expansion_factor[1]))

                roll_h = torch.randint(0, height // 8, (1,)).item()
                roll_w = torch.randint(0, width // 8, (1,)).item()
                if self._tileable:
                    latents = torch.roll(latents, shifts=(roll_h, roll_w), dims=(2, 3))
                    mask = torch.roll(mask, shifts=(roll_h, roll_w), dims=(2, 3))
                    masked_image_latents = torch.roll(masked_image_latents, shifts=(roll_h, roll_w), dims=(2, 3))

                # If patched diffusion
                if self._patched:
                    B = latents.shape[0]

                    # patch the latents, mask and masked_image_latents
                    latents, size_padded = self.patch_image(latents, patch_size=self.unet.config.sample_size, overlap=0.0)
                    mask, size_padded = self.patch_image(mask, patch_size=self.unet.config.sample_size, overlap=0.0)
                    masked_image_latents, size_padded = self.patch_image(masked_image_latents, patch_size=self.unet.config.sample_size, overlap=0.0)
                    ...
                    if prompt_embeds.shape[0] != Bp * 2:
                        ...
                        added_cond_kwargs["image_embeds"][0] = added_cond_kwargs["image_embeds"][0].repeat_interleave(Bp // B, dim=0)
                ...
                if self._patched:
                    # unpatch the latents
                    latents = self.unpatch_image(latents, B, size_padded, patch_size=self.unet.config.sample_size, overlap=0.0,)
                    mask = self.unpatch_image(mask, B * 2, size_padded, patch_size=self.unet.config.sample_size, overlap=0.0,)
                    masked_image_latents = self.unpatch_image(masked_image_latents, B * 2, size_padded, patch_size=self.unet.config.sample_size, overlap=0.0,)
